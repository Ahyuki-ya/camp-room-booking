...
参加者に部屋を左から詰めて使ってもらうため、**左に2部屋以上ある部屋（＝並び順で3番目以降）は、同じ枠で左隣の部屋が予約されるまで予約できない**。

- 1番目と2番目の部屋は常に予約できる。
- 判定は**同一スロット内**で行う。ある枠で3番目が開いていても、別の枠では独立に判定する。
...
> **この規則は新規の予約だけを制限する。** 3番目が予約された後に2番目がキャンセルされても、3番目の予約はそのまま残る。すでに成立した予約を後から無効にはしない。同様に、4番目は3番目が予約されている限り開いたままである。キャンセルによって一時的に「左が空いているのに右が埋まっている」状態が生じうるが、これは許容する。
...
4b. `rooms` から `p_room_id` の `sort_order` を得る。行がなければ `NO_SUCH_ROOM` を raise。`sort_order` がそれより小さい部屋が**2つ以上**あり、かつ**そのうち最大の部屋**（＝左隣）が同じ `p_start_at` で予約されていなければ `ROOM_LOCKED` を raise（FR-08）
...
sql/04_seed.sql     rooms 5件、slots 20件
...
insert into rooms (id, name, sort_order) values
  (1,'E6（大）',1),(2,'D1（中）',2),(3,'D3（中）',3),(4,'E1（中）',4),(5,'E2（中）',5);
...
25. **左から順に予約すれば全部屋が取れ、右端から予約しようとすると3番目以降が `ROOM_LOCKED` で拒否される**（FR-08）。
...
| 項目 | 結果 |
|---|---|
| 基準8（実機での表示・操作） | ✅ Android と iPhone の両方で確認。5列のグリッドが判読・操作できる |
...
| 3 | 部屋の名称 | **E6（大）／ D1（中）／ D3（中）／ E1（中）／ E2（中）**。**暫定**であり変更の可能性がある | 2026-08-27 に「部屋1〜5」から変更 |
...
| 16 | 部屋の順次開放 | **3番目以降の部屋は、同じ枠で左隣が予約されるまで開かない**。効率的に部屋を使ってもらうため | 2026-08-27 |
